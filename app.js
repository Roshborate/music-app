...
document.getElementById('search-keyword').addEventListener('input', function() {
  const keyword = document.getElementById('search-keyword').value;
  if (keyword) {
    searchYouTubeVideos(keyword);  // Trigger the search as user types
  }
});
...
function displaySearchResults(videos) {
  ...
  videos.forEach((video, index) => {
    ...
    // When a video is clicked, play it immediately and clear search results
    listItem.addEventListener('click', () => {
      currentVideoIndex = index; // Set current video index
      playVideo(video.id.videoId);  // Play the selected video
      resultsContainer.innerHTML = '';  // Hide search suggestions after selection
    });
...
// Play the selected video and update UI with thumbnail and title
// function playVideo(videoId) {
//   player.loadVideoById(videoId);  // Load the selected video by ID
//   player.playVideo();             // Play the video

//   // Clear the search results after selecting a video
//   document.getElementById('search-results').innerHTML = '';

//   // Find the current video in the list and update the thumbnail and title
...
//   // Set the song title
//   document.getElementById('video-title').textContent = currentVideo.snippet.title;
// }

// Modify the playVideo function to call updateNavigationButtons after a video is played
function playVideo(videoId) {
  player.loadVideoById(videoId);  // Load the selected video by ID
  player.playVideo();             // Play the video
  ...
  // Update the navigation buttons (Previous/Next)
  updateNavigationButtons();
}

// Play the previous video in the search results
function playPreviousVideo() {
  if (currentVideoIndex > 0) {
    currentVideoIndex--; // Go to the previous video
    const prevVideoId = currentVideos[currentVideoIndex].id.videoId;
    playVideo(prevVideoId);
  }
}
...
// Enable or disable the previous and next buttons based on the video index
function updateNavigationButtons() {
  document.getElementById('previous-button').disabled = currentVideoIndex === 0;
  document.getElementById('next-button').disabled = currentVideoIndex === currentVideos.length - 1;
}
